...
# Check 5-a, 6
5-a = You can easily check your result using dplyr's group_by and summarize.  
6 = Now use a looping construct (for or while) to loop through possible slice widths.  Report the various slice widths used, the sum calculated, and the slice width necessary to obtain an answer within $1e^{-6}$ of the analytical solution.
...
### b. A box plot of dev, by Observer (*?boxplot*).  
```{r box}
...
### c. A violin plot of dev by Observer (*??violin* two "?" will search through installed packages).
